  ...
  return (
    <div className="flex gap-0.5" aria-label={`${count} out of 5 stars`}>
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} className="text-gold text-sm" aria-hidden="true">
          ★
        </span>
      ))}
    </div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          {reviews.map((r, i) => (
            <div
              key={r.name}
              className="bg-blush border border-gold-light p-8 flex flex-col gap-4 fade-up"
              style={{ transitionDelay: `${i * 80}ms` }}
            >
              <Stars count={r.rating} />
              <p className="font-display text-xl font-light text-charcoal leading-relaxed italic flex-1">
                "{r.text}"
              </p>
              <div className="pt-4 border-t border-gold-light">
